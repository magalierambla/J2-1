...
public class projectController {
    ...
    @GetMapping("/visitor/projects_top_consulte")    
    public ResponseEntity<List<project>> getAllMyProjectsTopConsulteForVisistor() { 
    	
    	List<project> projects =  _projectRepository.findAllProjectsTopConsultation();
        ...
        if(projects.size()>0) {
        	...
        	for(int index=0;index<projects.size();index++) {
        		 ...
        		 // System.out.println("nom-user="+ projects.get(index).get_user().getNom());
        		
        		 _user.setNom(projects.get(index).get_user().getNom());
        		
        		 _user.setPrenom(projects.get(index).get_user().getPrenom());        		
    ...
    @GetMapping("/visitor/random_project")    
    public ResponseEntity<project> getRandomProjectForVisistor() { 
    	
    	project projectBdd =  _projectRepository.findProjectRandom();
    	
    	 adminstrateur  _manager_project = new adminstrateur();
    	
    	 user _user = new user();
    	 
    	 _user.setNom(projectBdd.get_user().getNom());
 		
		 _user.setPrenom(projectBdd.get_user().getPrenom());        		
		
		 projectBdd.setManager_project(_manager_project);
	 	 
		 projectBdd.set_user(_user);
    ...
    @GetMapping("/visitor/project/all_categories")    
    public ResponseEntity<List<category_project>> getAllCategoriesProjectVisiteur() { 
    	
    	List<category_project> categories =  _categorieProjectRepository.findCustumCategories();
   ...
   public ResponseEntity<project> updateProjectByAdmin(@RequestBody  project updateProject) {
	  ...
	  Optional<project>  projectBdd = _projectRepository.findById(updateProject.getId());   	 
  	 ...
  	 if(projectBdd.isPresent()) { 
        ...
        // System.out.println("statut-project-avant-update = "+ projectBddUpdate.get_statut_project().getNom()); 
  		 
  		 // System.out.println("statut-project-apres-update = "+ updateProject.get_statut_project().getNom()); 
  		 
  		Optional<category_project>  _category_project = _categorieProjectRepository.findByNom(updateProject.getCategoryProject().getNom());
  		...
  		if(_category_project.isPresent()) { 
  			...
  			System.out.println(updateProject.get_statut_project().getNom() == "Valide");
  		    
  			
  			 if(updateProject.get_statut_project().getId() == 2) {   // statut valide	  			 
  	  			
  			    	System.out.println("infos-categorie-project-attente-valide = "+ category_projectBdd.getNom() + '-' +category_projectBdd.getId() + '-' +category_projectBdd.getNbr_projects()  ); 
  	  		 ...
  	  		 if(updateProject.get_statut_project().getId() == 3) {  // statut annule
  	  			 
  	  			 if(projectBddUpdate.get_statut_project().getId() == 2) {  // statut valide
  	  				 
  	  				System.out.println("infos-categorie-project-annule-valide = "+ category_projectBdd.getNom() + '-' +category_projectBdd.getId() + '-' +category_projectBdd.getNbr_projects()  ); 
  	  	   ...
  	  	   System.out.println("infos-categorie-project-apres = "+ category_projectBdd.getNom() + '-' +category_projectBdd.getId() + '-' +category_projectBdd.getNbr_projects()  ); 
   ...
   public ResponseEntity<commentProject>  createComment(@PathVariable String token_user, @PathVariable String token_project, @RequestBody  commentProject _comment) {
	   ...
	   int nbr_comments = _project.getTotal_comments();
	   
	   
	   nbr_comments = nbr_comments +1;
	   
	   _project.setTotal_comments(nbr_comments);
  ...
  /**************************************************************************************/
   ...
   @PostMapping(value = "/users/{token_user}/projects/{token_project}/investisseurs_project/createDemandeInvest") 
   @ResponseBody
   public ResponseEntity<investisseursProjectModel>  createDemandeInvestProject(@PathVariable String token_project , @RequestBody   investisseursProjectModel _demandeInvestProject) {  	   
   ...
   @PutMapping(value = "/users/{token_user}/projects/{token_project}/investisseurs_project/{token_demande}/updateDemandeInvest") 
   @ResponseBody
   public ResponseEntity<investisseursProjectModel>  updateDemandeInvestProject(@PathVariable String token_project , @RequestBody   investisseursProjectModel _demandeInvestProject) {  
	   ...
	   Optional<investisseursProjectModel>  demandeInvestProjectBdd = _investisseursProjectRepository.findById(_demandeInvestProject.getId());   	 
	  	 ...
	  	 if(demandeInvestProjectBdd.isPresent()) { 
            ...
            boolean isEqual = (_demandeInvestProject.getStatutDemande().toString().equals("VALIDE"));
	  		
	  		System.out.println("_demandeInvestProject.getStatutDemande()="+ _demandeInvestProject.getStatutDemande()); 
	  		
	  		if(isEqual) {
	  			
	  			System.out.println("isEqual="+  isEqual);  
	  			
	  			int nbr_investisseurs_project = _project.getTotal_investisseurs();
	  			
	  			
	  			nbr_investisseurs_project = nbr_investisseurs_project +1;
	  			
	  			
	  			_project.setTotal_investisseurs(nbr_investisseurs_project);
